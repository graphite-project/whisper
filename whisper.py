...
def parseRetentionDef(retentionDef):
    ...
    if precision.isdigit():
        ...
        if match:
            precision = (
                int(match.group(1)) * UnitMultipliers[getUnitString(match.group(2))]
            )
        else:
    ...
    if points.isdigit():
        ...
        if match:
            points = (
                int(match.group(1))
                * UnitMultipliers[getUnitString(match.group(2))]
                // precision
            )
        else:
...
def __writeHeaderMetadata(
    fh, aggregationMethod, maxRetention, xFilesFactor, archiveCount
):
    ...
    try:
        ...
        raise InvalidXFilesFactor("Invalid xFilesFactor %s, not a float" % xFilesFactor)

    if xFilesFactor < 0 or xFilesFactor > 1:
        raise InvalidXFilesFactor(
            "Invalid xFilesFactor %s, not between 0 and 1" % xFilesFactor
        )
...
def create(
    ...
    with open(path, "wb", BUFFERING) as fh:
        try:
            ...
            __writeHeaderMetadata(
                fh, aggregationMethod, oldest, xFilesFactor, len(archiveList)
            )
            ...
            for secondsPerPoint, points in archiveList:
                archiveInfo = struct.pack(
                    archiveInfoFormat, archiveOffsetPointer, secondsPerPoint, points
                )
...
def __archive_update_many(fh, header, archive, points):
    ...
    alignedPoints = [
        (timestamp - (timestamp % step), value) for (timestamp, value) in points
    ]
...
def __archive_fetch(fh, archive, fromTime, untilTime):
    ...
    for i in xrange(0, len(unpackedSeries), 2):
        ...
        if pointTime == currentInterval:
            ...
            valueList[i // 2] = (
                pointValue  # In-place reassignment is faster than append()
            )
        currentInterval += step
...
def file_diff(fh_from, fh_to, ignore_empty=False, until_time=None, now=None):
    ...
    for archive_number, archive in enumerate(archives):
        ...
        (fromTimeInfo, fromValues) = __archive_fetch(
            fh_from, archive, startTime, untilTime
        )
        ...
        points = map(
            lambda s: (s * archive_step + start, fromValues[s], toValues[s]),
