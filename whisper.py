...
def parseRetentionDef(retentionDef):
    ...
    if points.isdigit():
        ...
        if match:
            points = int(match.group(1)) * UnitMultipliers[
                getUnitString(match.group(2))] / precision
        else:
...
def __readHeader(fh):
    ...
    try:
        (aggregationType, maxRetention, xff, archiveCount) = struct.unpack(metadataFormat,
                                                                           packedMetadata)
    except:
...
def setAggregationMethod(path, aggregationMethod, xFilesFactor=None):
  ...
  """
    ...
    try:
        ...
        try:
            newAggregationType = struct.pack(longFormat,
                                             aggregationMethodToType[aggregationMethod])
        except KeyError:
...
def create(path, archiveList, xFilesFactor=None, aggregationMethod=None, sparse=False,
  ...
  """
    ...
    try:
        ...
        aggregationType = struct.pack(longFormat,
        ...
        for secondsPerPoint, points in archiveList:
            archiveInfo = struct.pack(archiveInfoFormat, archiveOffsetPointer, secondsPerPoint,
                                      points)
...
def file_update(fh, value, timestamp):
    ...
    for i, archive in enumerate(
            header['archives']): #Find the highest-precision archive that covers timestamp
        if archive['retention'] < diff: continue
        lowerArchives = header['archives'][
                        i + 1:] #We'll pass on the update to these lower precision archives later
        break
...
def __archive_update_many(fh, header, archive, points):
    ...
    if baseInterval == 0: #This file's first update
        baseInterval = packedStrings[0][
            0] #use our first string as the base, so we start at the start
    ...
    for (interval, packedString) in packedStrings:
        ...
        if bytesBeyond > 0:
            ...
            fh.write(packedString[
                     -bytesBeyond:]) #safe because it can't exceed the archive (retention checking logic above)
        else:
            fh.write(packedString)

    #Now we propagate the updates to lower-precision archives
    higher = archive
    lowerArchives = [arc for arc in header['archives'] if
                     arc['secondsPerPoint'] > archive['secondsPerPoint']]
...
def __archive_fetch(fh, archive, fromTime, untilTime):
  ...
  """
    fromInterval = int(fromTime - (fromTime % archive['secondsPerPoint'])) + archive[
        'secondsPerPoint']
    untilInterval = int(untilTime - (untilTime % archive['secondsPerPoint'])) + archive[
        'secondsPerPoint']
    fh.seek(archive['offset'])
    packedPoint = fh.read(pointSize)
    (baseInterval, baseValue) = struct.unpack(pointFormat, packedPoint)
...
def file_diff(fh_from, fh_to, ignore_empty=False):
    ...
    for archive_number, archive in enumerate(archives):
        ...
        min(fromTimeInfo[0], toTimeInfo[0]), max(fromTimeInfo[1], toTimeInfo[1]),
